Select Ord_id, m.Ship_id, Ship_Date, Shipping_cost from market_fact_full as m inner join shipping_dimen as s on m.ship_id = s.ship_id;
Select customer_name, city, state, ord_id from market_fact_full as m inner join cust_dimen as c on m.cust_id = c.cust_id;
Select ord_id, m.ship_id, ship_mode from market_fact_full as m inner join shipping_dimen as s on m.ship_id = s.ship_id where ship_mode = "Regular air";
Select order_number, order_priority, case
when order_priority ="Critical" then "immediate delivery" when order_priority ="High" then "immediate delivery" else "normal delivery"
end as Delivery_type from orders_dimen;
...
insert into shipping_mode_dimen(ship_mode, vehicle_company, toll_required) values
...
group by city;
select distinct customer_namefrom cust_dimen
...
from orders_dimen
where order_priority = 'critical' group by Order_Year, order_month order by order_count desc ;
select ship_mode, count(ship_mode) as ship_mode_count from shipping_dimen
where year (ship_date) = 2011 group by ship_mode
order by ship_mode_count desc;
select customer_name, cust_id from cust_dimen
where cust_id = ( select cust_id
from market_fact_full group by cust_id order by count(cust_id) desc limit 1
);
...
RANK() OVER (ORDER BY sales DESC) AS sales_rank FROM market_fact_full as m
INNER JOIN
cust_dimen as c
...
RANK() OVER (ORDER BY discount ASC) AS disc_rank, DENSE_RANK() OVER (ORDER BY discount ASC) AS disc_dense_rank
FROM market_fact_full as m INNER JOIN cust_dimen as c ON m.cust_id=c.cust_id WHERE customer_name= 'Aaron Smayling';
SELECT customer_name,
...
order_row_num
FROM market_fact_full AS m INNER JOIN
cust_dimen AS c
ON m.cust_id=c.cust_id
WHERE customer_name= 'Aaron Smayling' GROUP BY customer_name ;
